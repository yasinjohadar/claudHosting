    ...
    <div class="container-fluid">
        <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
            ...
            <div class="ms-auto pageheader-btn d-flex gap-2">
                <a href="{{ route('admin.invoices.show', $invoice->id) }}" class="btn btn-info-light">
                    ...
                    <form action="{{ route('admin.invoices.update', $invoice->id) }}" method="POST" id="invoiceForm">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select @error('paymentmethod') is-invalid @enderror" id="paymentmethod" name="paymentmethod">
                                        ...
                                        <option value="banktransfer" @selected(old('paymentmethod', $invoice->paymentmethod) === 'banktransfer')>تحويل بنكي</option>
                                        <option value="creditcard" @selected(old('paymentmethod', $invoice->paymentmethod) === 'creditcard')>بطاقة ائتمان</option>
                                        <option value="paypal" @selected(old('paymentmethod', $invoice->paymentmethod) === 'paypal')>PayPal</option>
                                        <option value="cash" @selected(old('paymentmethod', $invoice->paymentmethod) === 'cash')>نقدي</option>
                                        <option value="other" @selected(old('paymentmethod', $invoice->paymentmethod) === 'other')>أخرى</option>
                                    </select>
                                    @error('paymentmethod')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control @error('duedate') is-invalid @enderror" id="duedate" name="duedate"
                                        value="{{ old('duedate', $invoice->duedate?->format('Y-m-d')) }}" required>
                                    @error('duedate')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="col-12">
                                    ...
                                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2">{{ old('notes', $invoice->notes) }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <table class="table table-bordered table-hover mb-0 align-middle" id="itemsTable">
                                    ...
                                    <tfoot class="table-light">
                                        <tr>
                                            ...
                                            <td class="fw-bold" id="totalAmount">{{ number_format($invoice->total, 2) }} ر.س</td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                        ...
                        <div class="card-footer d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="fe fe-save"></i> تحديث الفاتورة
                            </button>
                            <a href="{{ route('admin.invoices.show', $invoice->id) }}" class="btn btn-light">إلغاء</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <ul class="list-unstyled mb-0">
                            <li class="mb-2"><span class="text-muted">الرقم:</span> <strong>{{ $invoice->invoice_number }}</strong></li>
                            <li>
